...
def setup_snapshot_image_grid(training_set, random_seed=0, random=False):
    rnd = np.random.RandomState(random_seed)
    gw = np.clip(7680 // training_set.image_shape[2], 7, 32)
    gh = np.clip(4320 // training_set.image_shape[1], 4, 32)

    # No labels => show random subset of training samples.
    if not training_set.has_labels or random:
...
def training_loop(
    ...
    if rank == 0:
        ...
        grid_size, images, segs, labels = setup_snapshot_image_grid(training_set=training_set, random=True)
    ...
    while True:
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            ...
            save_image_grid(images_raw, os.path.join(run_dir, f'fakes{cur_nimg//1000:06d}_raw.jpg'), drange=[-1,1], grid_size=grid_size)
            save_image_grid(images_depth, os.path.join(run_dir, f'fakes{cur_nimg//1000:06d}_depth.jpg'), drange=[images_depth.min(), images_depth.max()], grid_size=grid_size)
            save_image_grid(images_alpha, os.path.join(run_dir, f'fakes{cur_nimg//1000:06d}_alpha.jpg'), drange=[0,1], grid_size=grid_size)
            if stats_tfevents is not None:
                global_step = int(cur_nimg / 1e3)
                stats_tfevents.add_image(f'fakes', tvu.make_grid(
                    torch.tensor(images[:16]),
                    nrow=min(4,int(math.ceil(images.shape[0] ** 0.5))),
                    normalize=True,
                    value_range=(-1,1),
                ), global_step)
                stats_tfevents.add_image(f'fakes_raw', tvu.make_grid(
                    torch.tensor(images_raw[:16]),
                    nrow=min(4,int(math.ceil(images.shape[0] ** 0.5))),
                    normalize=True,
                    value_range=(-1,1),
                ), global_step)
                stats_tfevents.add_image(f'fakes_depth', tvu.make_grid(
                    torch.tensor(images_depth[:16]),
                    nrow=min(4,int(math.ceil(images.shape[0] ** 0.5))),
                    normalize=True,
                    value_range=(images_depth.min(), images_depth.max()),
                ), global_step)
                stats_tfevents.add_image(f'fakes_alpha', tvu.make_grid(
