...
class StyleGAN2Loss(Loss):
    ...
    def run_G(self, z, c, swapping_prob, neural_rendering_resolution, update_emas=False):
        ...
        # c_new is the c with offset
        c_new, delta_c = self.G.apply_delta_c(z, c, update_emas=update_emas)
        gen_output = self.G.synthesis(ws, c_new, neural_rendering_resolution=neural_rendering_resolution, update_emas=update_emas, ws_bcg=ws_bcg)
        return gen_output, ws, delta_c
    ...
    def run_D(self, img, c, blur_sigma=0, blur_sigma_raw=0, update_emas=False):
        ...
        if self.augment_pipe is not None:
            raise NotImplementedError("augmentation for segmentation is not implemented.")
    ...
    def accumulate_gradients(self, phase, real_img, real_seg, real_c, gen_z, gen_c, gain, cur_nimg):
        ...
        self.G.rendering_kwargs["density_noise"] = max(1 - cur_nimg / (self.density_noise_fade_kimg*1e3), 0) if self.density_noise_fade_kimg > 0 else 0
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws, delta_c = self.run_G(gen_z, gen_c, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution)
            ...
            # Camera pose self-adaptation loss
            loss_trans_reg = 0
            if self.G.rendering_kwargs['trans_reg'] > 0:
                loss_trans_reg = (delta_c.mean(0)**2).sum() * self.G.rendering_kwargs['trans_reg']
                training_stats.report('Loss/G/trans_reg_loss', loss_trans_reg) 

            with torch.autograd.profiler.record_function('Gmain_backward'):
                (loss_Gmain + loss_trans_reg).mean().mul(gain).backward()
        ...
        if phase in ['Greg', 'Gboth'] and self.G.rendering_kwargs.get('density_reg', 0) > 0 and self.G.rendering_kwargs['reg_type'] == 'monotonic-detach':
            ...
            training_stats.report('Loss/G/monotonic_loss', monotonic_loss)
        ...
        if phase in ['Dmain', 'Dboth']:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, _gen_ws, delta_c = self.run_G(gen_z, gen_c, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution, update_emas=True)
